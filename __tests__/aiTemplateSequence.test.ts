...
import { buildRequiredTemplateSequence, resolveSlideBoundsFromEnv } from '../src/pipeline/aiService';
...
describe('ai template sequence selection', () => {
  ...
  it('builds a bounded sequence with HOOK_A first and CTA_FINAL last', () => {
    ...
    expect(middle.every((id) => id === 'CONTENT_LISTICLE' || id === 'CONTENT_GENERIC' || id === 'CONTENT_VIDEO')).toBe(true);
  });
