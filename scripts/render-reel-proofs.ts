...
const PROOF_DIR = path.resolve(__dirname, '..', 'proof-frames', 'reel');
...
// Ensure proof-frames/reel directory exists
if (!fs.existsSync(PROOF_DIR)) {
    fs.mkdirSync(PROOF_DIR, { recursive: true });
}

console.log('🎬 Rendering editorial reel proof frames (1080×1920 / 9:16)...\n');

for (const niche of NICHES) {
    const manifestPath = path.join(TEST_MANIFESTS_DIR, `reel-${niche}.json`);
    ...
    const frame0Path = path.join(nicheDir, 'reel-frame-0.png');
    console.log(`   → Rendering frame 0 (initial)...`);
    try {
        execSync(
            `npx remotion still src/remotion/index.tsx EditorialReel "${frame0Path}" --frame=0 --props="${tempPropsFile}"`,
    ...
    const frame24Path = path.join(nicheDir, 'reel-frame-24.png');
    console.log(`   → Rendering frame 24 (settled)...`);
    try {
        execSync(
            `npx remotion still src/remotion/index.tsx EditorialReel "${frame24Path}" --frame=24 --props="${tempPropsFile}"`,
            { cwd: path.resolve(__dirname, '..'), stdio: 'pipe' }
        );
...
console.log('✨ Editorial reel proof frames rendered successfully!');
